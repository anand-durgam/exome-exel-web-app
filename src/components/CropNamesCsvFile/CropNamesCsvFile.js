...
import './CropNamesCsvFile.css'
import React, { useState } from "react";
import Modal from 'react-bootstrap/Modal'

const CropNamesCsvFile = () => {
    ...
    const csvFileToArray = string => {
      ...
      const cropNamesArray = csvRows.map(item => item)
    //   console.log(cropNamesArray)

      function postCropNamesAPI(item){
        console.log(item)
        ...
        const cropLanguageName = secondPart.slice(0,-1)
        console.log(cropLanguageName)
        ...
        body: JSON.stringify([{
            label: cropName,
            value: cropName,
            language: cropLanguageName,
        }])
        };
        
        fetch("http://10.0.0.237:3003/crops/create2", options)
      ...
      cropNamesArray.map(item => postCropNamesAPI(item))
...
export default CropNamesCsvFile
